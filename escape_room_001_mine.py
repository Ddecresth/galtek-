...
def listformat(object_list):
	l=["a+"obj.name for object in object_list if object ["visible"]]
	return ",".join(l)

def myPrint(cmd,client):
	client_gameplay(client,str(cmd).encode())

def client_gameplay(client,cmd,RECV_SIZE=1024):
	client.send(cmd+b'<EOL>\n')
	from_server=client.recv(RECV_SIZE).decode("utf-8")
	lines=from_server.split('\n')
	print(lines)
	time.sleep(0.28)
...
class EscapeRoomCommandHandler:
    def __init__(self, room, player, output=myPrint):
        self.room = room
        self.player = player
        self.output = output
	self.clien]t=client
    ...
    def _cmd_look(self, look_args):
        ...
        self.output(look_result,self.client)
    ...
    def _cmd_unlock(self, unlock_args):
        ...
        self.output(unlock_result,self.client)
    ...
    def _cmd_open(self, open_args):
        ...
        """
        if len(open_args) == 0:
            return self.output("Open what?",self.client)
        ...
        self.output(open_result,self.client)
    ...
    def _cmd_get(self, get_args):
        ...
        self.output(get_result,self.client)
    ...
    def _cmd_inventory(self, inventory_args):
        ...
        if len(inventory_args) != 0:
            self.output("What?!",self.client)
            return
            
        items = ", ".join(["a "+item for item in self.player["container"]])
        self._run_triggers(object, "inventory")
        self.output("You are carrying {}".format(items),self.client)
        
    def command(self, command_string):
        # no command
        if command_string.strip == "":
            return self.output("",self.client)
        ...
        if not hasattr(self, function):
            return self.output("You don't know how to do that.",self.client)
...
class EscapeRoomGame:
    def __init__(self, command_handler_class=EscapeRoomCommandHandler, output=myprint):
    ...
    def start(self):
        self.status = "playing"
        self.output("Where are you? You don't know how you got here... Were you kidnapped? Better take a look around",self.client)
        
    def command(self, command_string):
        if self.status == "void":
            self.output("The world doesn't exist yet!",self.client)
        elif self.status == "created":
            self.output("The game hasn't started yet!",self.client)
        elif self.status == "dead":
            self.output("You already died! Sorry!",self.client)
        elif self.status == "escaped":
            self.output("You already escaped! The game is over!",self.client)
        else:
            ...
            if not self.player["alive"]:
                ...
                self.output("VICTORY! You escaped!",self.client)
